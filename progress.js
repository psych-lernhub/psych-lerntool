...
(function () {
  ...
  function typeOf(nn) {            // Suffix NN -> [Label, Sortier-Rang]
    nn = parseInt(nn, 10);
    if (nn === 0 || nn === 91 || nn === 94) return ["Altklausur / Set", 5];   // 94: Lernfragen-Set (m5, 14.07.)
    if (nn === 90) return ["Modell-Trainer", 1];
    if (nn === 98 || nn === 92 || nn === 93) return ["KI-Set", 4];            // 93: KI-Übungsset 3 (m5, 14.07.)
    if (nn === 95 || nn === 96) return ["Klausurformat-Übung", 3];             // m11a Kprim/Malus-Sets (95: ak44-Voll-Probeklausur, ergänzt 31.07.)
    if (nn === 97) return ["Freitext-Set", 6];
    if (nn === 99) return ["OSCE-Simulation", 7];
    if (nn >= 21 && nn <= 29) return ["Vertiefung", 2];
    // 30–89 = Übungsset-Bereich (VL-Module gehen projektweit nie über 25). Erweitert
    // 17.07.2026: die M4b-Teil-Sets und m12b vl80 lagen zuvor fälschlich unter „VL".
    if (nn >= 30 && nn <= 89) return ["KI-Set", 4];
